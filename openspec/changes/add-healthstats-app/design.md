...
Since Health Connect exposes no reliable per-kilometer split/lap data for common sources (confirmed absent for Strava), "5K" is necessarily a tolerance bucket (default ±1.0 km, user-adjustable) applied to each session's total aggregated distance — not an exact-match or a derived-from-splits calculation. The default was widened from an initial ±0.2 km after real-world testing showed that value too narrow to surface any matches for typical untracked-distance runs. This is a modeling choice forced by the data source, not an implementation detail; it's reflected in the trends spec as "target ± tolerance."
